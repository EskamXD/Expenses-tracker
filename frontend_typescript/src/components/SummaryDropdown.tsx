 ...
 */
const SummaryDropdown: React.FC<SummaryDropdownProps> = ({
    selectedOwner,
    setSelectedOwner,
}) => {
     ...
     */
    const handleSummaryChange = (newOwner: number) => {
        if (setSelectedOwner) {
    ...
    return (
        <Dropdown id="dropdown-basic-button-summary">
            <Dropdown.Toggle>
                Wybierz podsumowanie: {getPersonOption(localOwner)}
            </Dropdown.Toggle>
            <Dropdown.Menu>
                {Object.entries(selectPersonOptions).map(([key]) => {
                    return (
                        <React.Fragment key={`fragment-${key}`}>
                            <Dropdown.Item
                                ...
                                onClick={() =>
                                    handleSummaryChange(parseInt(key))
                                }>
