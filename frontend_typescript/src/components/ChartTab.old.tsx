...
const generateDatesForSelectedMonth = (year: number, month: number) => {
    ...
    return Array.from({ length: daysInMonth }, (_, i) =>
        new Date(year, month - 1, i + 1).toLocaleDateString("pl-PL")
    );
};
...
const ChartTab = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            if (linearExpenseSums.length !== dates.length) {
                console.warn(
                    "Length mismatch: ",
                    linearExpenseSums.length,
                    dates.length
                );
            }
            ...
            console.log(
                "Lenght of arrays: ",
                linearExpenseSums.length,
                linearIncomeSums.length,
                lineSumsTrendXAxis.length,
                fetchedBarPersons.length,
                fetchedBarShops.length
            );
                    ...
                    {selectedOwner !== -1 && (
                        <ThemeProvider theme={theme}>
                            ...
                            <div
                                ...
                                <LoadingChart
                                    isLoading={
                                        !itemsLoaded && loading.barShopsChart
                                    }
                                    chartComponent={
                                        <BarChart
                                            yAxis={[
                            ...
                            <div className="d-flex full-w align-items-center">
                                {/* <LoadingChart
                                    isLoading={!itemsLoaded && loading.pieCategoriesChart}
                                    chartComponent={
                                        <PieChart
                                            series={[
                                                {
